...
resource "aws_db_parameter_group" "rtc_rds_parameter" {
    name = "rtc-rds-parameter"
...
resource "aws_db_instance" "three_rds" {
    ...
    parameter_group_name = aws_db_parameter_group.rtc_rds_parameter.name # rds instance 파라미터 그룹 이름
    ...
    db_subnet_group_name = aws_db_subnet_group.rds_subnet_group.id # db 인스턴스가 속한 서브넷 그룹 이름
